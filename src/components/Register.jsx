...
const Register = ({ onRegistered }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !email || !password) return setError("⚠️ All fields are required.");

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) return setError("⚠️ Please enter a valid email.");

    if (password.length < 8) return setError("⚠️ Password must be at least 8 characters.");
    ...
    try {
      ...
      if (res.ok) {
        const returnedUid = data?.user?.uid;
        if (returnedUid) {
          try {
            localStorage.setItem("uid", returnedUid);
          } catch {}
        }
        ...
        if (typeof onRegistered === "function") onRegistered(data);
      } else {
        const serverMsg = data?.message || (data?.error || "Something went wrong");
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="name" className="block text-sm text-gray-600 font-medium">
                Name
              </label>
              <input
                id="name"
          ...
          <p className="text-center text-sm text-gray-500">
            Already a user?{" "}
            <a href="/login" className="text-gray-700 font-medium hover:text-pink-500 transition-colors">
              LOGIN
            </a>
          </p>
        </div>
